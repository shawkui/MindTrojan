...
'''
Train Model on given Poison Datasets
'''
...
def main(args):
    ...
    os.makedirs(f"record/{args.data}_{args.model}_{args.poison_data}", exist_ok=True)

    # Set logging
    args.log = f'record/{args.data}_{args.model}_{args.poison_data}/log'
    os.makedirs(args.log, exist_ok=True)
    set_logging(args)

    train_data = dataset_bd_train
    ...
    for epoch in range(args.num_epoch):
        ...
        if epoch % 10 == 0:
            ms.save_checkpoint(net, f"record/{args.data}_{args.model}_{args.poison_data}/latest_model.ckpt")

    # Step 4: Save model
    ms.save_checkpoint(net, f"record/{args.data}_{args.model}_{args.poison_data}/attack_model.ckpt")
